...
_OHLCV_FEATURES: List[str] = [
    ...
    "dist_52w_high",                  # % below 52w high, in [-1,0]
...
# (label_5d/label_30d/label_60d); prepare_final.py renames the chosen horizon to
# return_<h>d and DROPS the others so there's no look-ahead leak.
# feature_set='final' points Config at these parquets + the lists below.
#
# left out on purpose:
#   - raw price levels (open/high/low/close/volume/vwap, sma_*, ema_*) are
#     non-stationary; I keep the stationary versions instead (log_price_z,
#     price_to_sma*, *_ret).
...
_FINAL_STOCK_FEATURES: List[str] = [
    ...
    # dominate the input norm — that pushed every stock's embedding into the
    # same direction (input cosine 0.97 -> 0.32 once dropped). if I want
...
# test Sharpe and the biggest val->test gap). the time-varying sector aggregates
...
# out-of-sample ceiling — the bottleneck is the regime/data, not the inputs.
...
class Config:
    ...
    #   feature_set ∈ {'ohlcv', 'fundamentals', 'news'}
    #   target_horizon ∈ {5, 30}
    # together they map to one of 6 parquets:
    #   data/stocks_<feature_set>_<horizon>d.parquet
    # e.g. feature_set='news', target_horizon=5 → data/stocks_news_5d.parquet
    #
    ...
    # going from ~75 val days (504) to ~340 (full) roughly halves the SE of the
    # mean Rank-IC. pass --last-n-days 504 for a quick smoke test.
    ...
    label_col: str = "label_up"              # binary label, still loaded for the direction-acc metric
    ...
    # 5d cumulative raw return has std ~0.04-0.05, so ×20 brings it to ~1.
    ...
    # to the train mean, pred_std -> 0, dir_acc -> 0.5).
    #
    ...
    # cross-sectional target — amplifies the signal + makes it a ranking problem.
    ...
    loss_type: str = "huber"
    huber_delta: float = 1.0     # the kink, in std-units (~1σ)
    ...
    # "no_graph"      encoder → head only, no GraphConstructor/GAT.
    ...
    # ticker×ticker adjacency (indexed + columned by ticker).
    ...
    # ---- windowing -----------------------------------------------------------
    # bumped window 90 -> 180 (~8.5 trading months of context). had to bump
    # patch_len too to keep n_patches sane: at W=180, patch_len=10 -> 18 patches
    ...
    # stock prediction (DTML is d=256/8 layers, PatchTST ref is d=128/3-4) — I
    # sit in the middle.
    #
    ...
    n_heads_tst: int = 8                     # 16-dim per head (was 4 × 16)
    ...
    # ---- macro encoder (mini-PatchTST → regime vector r_t) ------------------
    ...
    # dropped the macro-conditioned ε threshold and macro-FiLM on Q/K. each was a
    # tiny zero-init nudge to the graph similarity surface that, with how noisy
    ...
    # sim = (W_q g_t) · (W_k g_t)ᵀ / √d_attn             (Q/K bilinear similarity)
    # ε_t = softplus(eps_base + W_ε · r_t)               (macro-conditioned threshold)
    # A_new = σ((sim − ε_t) / τ)                         (soft ε-sphere)
    # A_t = α · A_{t-1} + (1 − α) · A_new                (edge memory — GRU idea on edges)
    ...
    # toggle the GRU step in the GraphConstructor. False = h ← z (no temporal
    ...
    # dropped this 0.3 -> 0.1.  A_t = α·A_{t-1} + (1-α)·A_new
    ...
    edge_ema: float = 0.1
    # 0 = pure soft ε-sphere (dense — sigmoid never hits zero so every pair has
    # some weight). >0 = also keep only top-k neighbours per row (applied AFTER
    # the edge-EMA so the smoothing still feeds the cut). default 5 -> each stock
    # attends to ~5% of the universe. GAT still respects edge WEIGHTS
    # (alpha = softmax(attn)·A) so even inside the top-5 it's proportional to similarity.
    topk: int = 5
    # was 5e-4, bumped to 2e-3 to keep the graph sparser — the richer input set
    # tempts the constructor into fitting spurious correlations.
    ...
    # ---- GAT -----------------------------------------------------------------
    # 4 heads × 32-dim each (d_model=128 / 4).
    # HPO knocked n_layers_gat 3 -> 2: the autoencoder probe + M3 audit showed a
    # 3-layer GAT over-smooths (post-graph embedding cosine 0.02 -> 0.74, a 3-hop
    ...
    n_heads_gat: int = 4
    n_layers_gat: int = 2                    # HPO: 3 -> 2 (over-smoothing fix)
    ...
    # comparable per-stock reps (drives embedding cosine ~0.99 -> ~0). this is the
    # representation-level half; dropping the calendar features above is the
    # input-level half. # WORKING
    ...
    # which is exactly why the graph clusters everything. N × d_model params is nothing.
    ...
    # decay=0.995 -> effective window ~200 steps ~8 epochs at last_n_days=504 /
    ...
    # for ListMLE.)
    # lr 1e-4 -> 3e-4. Optuna's cheap-proxy winner was ~1.7e-3 but that COLLAPSED
    # at full budget (pred_std -> 0, training flatlined) — classic short-horizon
    ...
    # weight_decay 5e-4 -> 1e-4. the lr-stability sweep paired stable lr=3e-4 with
    ...
    # — stops big early steps locking the model into a lucky-noise region.
    grad_clip: float = 1.0
    tbptt_steps: int = 10
    ...
    #   0.2 – 0.5     adds encoder signal without drowning target-loss; try with
    ...
    #                          → "predict value AND rank well". ratio set by
    ...
    #                          log-variances σ_h, σ_r on the model;
    #                          loss = Σ_k [exp(-s_k)·L_k + s_k], s_k = log σ_k². the
    ...
    #                          the listwise order already covers sign, so it'd be a
    #                          redundant extra weight. the big difference vs legacy
    ...
    # the cross-sectional median today" (the target's zero-mean per day). logit =
    # the head's score.
    ...
    #   huber_weight=1, aux_rank_weight=2  → 1:2 (rank-leaning)
    #   huber_weight=2, aux_rank_weight=1  → 2:1 (value-leaning)
    ...
    # target. default λ=0.6 on ranking (grid {0.5, 0.7} per Kwiatkowski & Chudziak
    # CIKM '25; pick the final λ on the PORTFOLIO metric you actually trade, since
    # max-IC != max-Sharpe). ListMLE's loss VALUE (~5) dwarfs Huber's (~0.35) but
    # their GRADIENTS are comparable (~0.049 vs ~0.032), so at λ=0.6 the Huber still
    # drives ~30% of the gradient — a real magnitude anchor. mag_weight=0 = pure
    # ranking. separate from huber_weight/aux_rank_weight, which only the legacy
    # single-target modes read.
    rank_weight: float = 0.6
    mag_weight:  float = 0.4

    # ListMLE listwise-ranking loss over all valid stocks per day:
    #     L_rank = -Σ (s_i − logsumexp(s_i, s_{i+1}, …, s_n))
    ...
    # when sign(pred) != sign(y) the squared error gets ×(1 + dir_loss_weight).
    # 0 = plain MSE; 1 = wrong-direction errors cost 2× the right-direction same
    # magnitude; 2 = 3×.
    # dir_hinge_weight adds a relu(-pred·y) term on top, which gives a clean
    # directional gradient even when |pred|,|y| are both tiny (where (pred-y)²
    ...
    def __post_init__(self):
        ...
        # legacy use_news still wired: True → 'news', False → 'fundamentals'
        ...
        if self.feature_set in ('final', 'final_v5'):
            # the final dataset (+ the v5 augmented variant). 5/30/60-day horizons.
            ...
            # return_30d std~0.095 -> ×10; return_60d std~0.134 -> ×7.5;
            # return_5d std~0.040 -> ×25. only if the default's untouched.
